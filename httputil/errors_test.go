...
func TestErrorHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{"validation errors public", context.Background(), validation.NewError("name", validation.Error{Message: "public message", Source: errors.New("private error")}), "", 400, `{"code":400,"message":"validation errors","fields":{"name":["public message"]}}`, "name: public message: private error."},
		{"validation errors json", context.Background(), validation.NewError("name", validation.Error{Message: "json error", Source: json.Unmarshal([]byte("bad json"), &nop)}), "", 400, `{"code":400,"message":"validation errors","fields":{"name":["json error"]}}`, "name: json error: invalid character 'b' looking for beginning of value."},
