...
const errFormat = "%s parameter invalid %s : %q : %s"
...
func GetInt32(r *http.Request, name string, required bool) (*int32, error) {
	...
	if err != nil {
		return nil, validation.New(name, fmt.Sprintf(errFormat, name, "int32", s, err)) //nolint: wrapcheck
	}
...
func GetTime(r *http.Request, name string, required bool) (time.Time, error) {
	...
	if err != nil {
		return time.Time{}, validation.New(name, fmt.Sprintf(errFormat, name, "date", s, err)) //nolint: wrapcheck
	}
...
func GetInt32Array(r *http.Request, name string, required bool) ([]int32, error) {
	...
	for i, p := range pp {
		...
		if err != nil {
			return nil, validation.New(name, fmt.Sprintf(errFormat, name, "int32", p, err)) //nolint: wrapcheck
		}
